              ...
              for(j=0;j<translations.length;j++)
              {
                ...
                //    console.log("languagues",languagues);
                    if (languagues == "hi") {
                         ...
                         data[i].animal_name=translations[j].values;
                    }
                    if (languagues == "en") {
                      console.log("test",translations[j].values);
                      data[i].animal_type=translations[j].values;
...
router.post('/add_orders',function(req,res){
  ...
  let customer_id = req.body.customer_id;
  let client_id = req.body.client_id;
  let messages=  setUserLangValue['order_successful'];
 ...
 try{
      ...
      if (rows.length > 0) {
        res.json({
          status:false,
          message:  setUserLangValue['order_already_added']
        });
      }else
      {
        ...
        console.log(insertRow);
          ...
          if(insertRow)
          {
              res.json({
              status:true,
              message:  setUserLangValue['added_order_successfuly']
            });
 ...
 try{
   ...
   sequelize.query("SELECT * FROM customers WHERE id=?  ",{type:sequelize.QueryTypes.SELECT, raw:true,  replacements: [customer_id]}).then(data => {
    if (data.length > 0) {
     ...
     let no_of_cattle=number_of_cattle_to_buy-stock_id.length;
     }else
     {
        res.json({
        status:false,
